...
`flood/data/r8/flood_xml.json` は取得する。ただし河川洪水予報はタクシー運用上の通知ノイズを避けるため、Discord通知はレベル3相当以上に限定する。

河川洪水予報:

- state保存: レベル2を含めて保存してよい
- Discord通知: レベル3相当以上のみ
- 通知対象:
  - `item.code >= "30"`
  - または `item.name` / `item.condition` に以下を含む
    - `氾濫警戒情報`
    - `氾濫危険情報`
    - `氾濫発生情報`
- 通知不要:
...
`warning/data/r8/230000.json` 側:

- 大雨・高潮などは `significancyPart.locals[].code >= "21"` を通知対象にする
- 土砂災害はレベル2相当をstate保存のみ、レベル3以上をDiscord通知対象にする
- 雷注意報は新規発表のみ必要なら通知対象にする
- 濃霧注意報は新規発表時のみ通知対象にする。継続中は通知しない。解除時は解除通知する。state保存対象。
...
これらは取得・state保存はしてよいが、Lv15.4ポリシーでは `item.code == "20"` のためDiscord通知対象外。
...
スクショの「愛知県日光川水系 日光川 レベル2氾濫注意情報」はこのJSONから取得できる。ただし通知はしない。
...
```text
landslide_suppressed: level=2 code=29 reason=policy
```
...
濃霧注意報はタクシー運用上、視界不良による郊外・高速・橋・河川沿い・深夜早朝の危険があるため通知対象にする。
...
1. `warning/data/r8/230000.json`
   ...
   - `kinds[].code in {"03","04","05","08","10","14","15","18","19","20","29"}`
   - `status in {"発表","継続"}` を active とする
   - `status == "解除"` は解除候補
   - `code == "20"` は濃霧注意報。新規発表と解除のみ通知し、継続中は通知しない。
   - `code == "29"` かつ `significancyPart.locals[].code == "21"` は土砂災害レベル2。state保存のみでDiscord通知しない。
   - `code == "29"` かつレベル3以上は新規発表と解除のみ通知し、継続中は通知しない。
2. `flood/data/r8/flood_xml.json`
   - `"2310000" in class20Codes`
   - stateには `レベル２氾濫注意報` も保存してよい
   - Discord通知は `item.code >= "30"` または `氾濫警戒情報` / `氾濫危険情報` / `氾濫発生情報` を含むものだけ
   - `item.code == "20"`、`レベル２氾濫注意報`、`氾濫注意情報` は通知しない
